...
LIBDIR:= lib /usr/local/lib /opt/ibm/ILOG/CPLEX_Studio128/cplex/lib/x86-64_linux/static_pic /opt/ibm/ILOG/CPLEX_Studio128/concert/lib/x86-64_linux/static_pic
OBJDIR:= obj
SRCDIR:= src
TESTDIR:= test
INCLDIR:= include . /usr/local/include /opt/ibm/ILOG/CPLEX_Studio128/cplex/include /opt/ibm/ILOG/CPLEX_Studio128/concert/include
BINDIR:= bin
SCANDIR:= scan
...
LDFLAGS:= -lemon -lconcert -lilocplex -lcplex -lpthread -ldl
...
LIBDIRCOMMAND=$(foreach PATH, $(LIBDIR), -L$(PATH))
...
$(BINDIR)/$(PLATFORM)/$(CONFIG)/$(TESTDIR)/%: $(OBJDIR)/$(PLATFORM)/$(CONFIG)/$(TESTDIR)/%.$(OBJEXT)
	$(SILENT) mkdir -p $(@D)
	$(SILENT) $(LD) $(LIBDIRCOMMAND) $^ -o $@ $(LDFLAGS)
